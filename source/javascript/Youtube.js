...
define("Youtube",
        ...
        function(t , $ , Channel , Video) {
			var Youtube = {
				subscribtions: {}, //channels by Id
				subscribtionsVideos: [], //videos by uploadDate
						...
						request.then(function(response){
							...
							for(k in response.result.items){
								if(response.result.items[k].snippet.assignable){
									...
									console.log(response.result.items[k]);
								}
							}
							console.log(response);
							resolve(categories);
				...
				subscriptionsList: function(channelId, pageToken){
					...
					var promise = new Promise(function(resolve, reject){
						...
						function(response){
							...
							console.log(response);
							resolve({channels : channels , nextPageToken : null });
						})
					})
					return promise;
				},
				/*
					Returns a Promise -> then a Array with 15 videos of the given playlist
				*/
				playlistItems: function(playlistId){
					...
					var promise = new Promise(function(resolve, reject){
						request.done(function(data){
							...
							Promise.all(requests).then(function(){
								...
								console.log(Youtube.subscribtionsVideos);
								resolve();
							})
						})
				...
				init: function(){
					var subscriptionsListLoaded = new Promise(function(resolve, reject){
						...
						var channels = [extra];
						var addSubscribtions = function(pageToken){
							...
							request.then(function(response){
								...
								if(response.nextPageToken){
									addSubscribtions(response.nextPageToken);
								}else{
						...
						var addChannel = function(channel , trys){
							Youtube.subscribtions[channel.id] = channel;
							var videoRequest = Youtube.playlistItems(channel.uploadId);
							videoRequest.then((function(videos){
								var y;
								for(y in videos){
									Youtube.subscribtions[videos[y].authorId].videos.push(videos[y]);
								}
							}),
								(function(response){
