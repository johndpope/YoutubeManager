...
function handleAuthResult( authResult , callBack ) {
	if (authResult && !authResult.error) {
		require("YoutubeService").loadRecommendations().then(function(){
			require("YoutubeService").authorized = true;
			if(callBack){
				callBack();
			}
		});
	} else {
		require("YoutubeService").authorized = false;
	}
}
			...
			var YoutubeService = {
				...
				*/
				...
				loadFullPlaylist: function(playlistId){
					...
					YoutubeService.Playlist = new Promise(function(resolve, reject){
				...
				loadRecommendations: function(pageToken){
					...
					var promise = new Promise(function( resolve , reject){
						request.then(function(response){
							...
							if(response.result.nextPageToken && videos.length <= 32){
								YoutubeService.loadRecommendations(response.result.nextPageToken).then(function(results){
					...
					promise.then(function(videos){
						YoutubeService.recommendations = videos;
					});
					return promise;
				},
					...
					var promise = new Promise(function(resolve, reject){
						request.done(function(data){
							...
							for(k in data.subscriptions){
								...
								var uploadRequest = YoutubeService.playlistItems(playlistId);
								uploadRequest.then(function(response){
									...
									for(y in videos){
										if(series.some(function(serie){ return videos[y].title.toLowerCase().indexOf(serie.title.toLowerCase()) != -1 }) ){
											YoutubeService.subscriptionsVideos.push(videos[y]);
										}
									}
								});
				...
				init: function(){
					var subscriptionsListLoaded = new Promise(function(resolve, reject){
						...
						var addChannel = function(channel , trys){
							YoutubeService.subscriptions[channel.id] = channel;
							var videoRequest = YoutubeService.playlistItems(channel.uploadId);
							videoRequest.then((function(response){
								var y;
								var videos = response.videos.slice(0, 16) //take only 16 videos
